...
class DemoService extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    case HttpRequest(POST, Uri.Path("/slack"), headers, entity, _) =>
      log.info(headers.toString)
      log.info(entity.toString)
      sender ! HttpResponse(entity = s"slack command: $headers\n entity: ${entity.toString}")

    //case HttpRequest(GET, Uri.Path("/slack"), _, _, _) =>
    //  sender ! HttpResponse(entity = s"slack command")
